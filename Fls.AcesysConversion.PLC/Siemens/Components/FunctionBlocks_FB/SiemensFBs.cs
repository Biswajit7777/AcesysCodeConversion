...
{
    public class SiemensFBs : SiemensCollection
    {
        public SiemensFBs(string prefix, string localname, string nsURI) : base(prefix, localname, nsURI)
        {
        ...
        public override void UpgradeVersion(SiemensProject Project, SiemensUpgradeOptions options, IProgress<string> progress)
        {
            ...
            if (awlContent != null)
            {
                processor = new UpgradeEngineFactorySiemens()
                                ...
                                .SetUpgradeProperties(options.FromVersion, options.ToVersion, Project.Manufacturer, typeof(SiemensFBs))
                                .Create();
            }
